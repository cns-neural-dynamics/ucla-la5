...
all_clusters = ['3', '4', '5', '6', '7', '8', '9', '10']
tasktype='rest'
server = libtmux.Server()

for ncluster in all_clusters:
    cmd1 = "tmux new-session -d -s cluster%s_gs 'python main_analysis.py -n 1 -r -a -c --analysis-type %s --data-analysis-type graph_analysis --window-type sliding --network-type full_network --ica_aroma-type nonaggr --glm_denoise --nclusters %s --rand-ind 20 --group-analysis-type ttest'" \
          %(ncluster, tasktype, ncluster)
    print cmd1
    subprocess.check_call(cmd1, shell=True)
    curr_gs = ''.join(['cluster', ncluster, '_gs'])
    while server.find_where({'session_name':curr_gs}):
        time.sleep(15)

# check if golden subjects is still runing, in case yes, wait
last_gs = ''.join(['cluster', all_clusters[-1], '_gs'])
